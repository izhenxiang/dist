...
var Le = ir(nn),
	ln = ir(an),
	ze = typeof globalThis != "undefined"
		? globalThis
					...
					$t(
						{
							...
							hasShadow: !1,
							transparent: !0,
							webPreferences: { preload: H.join(ft, "u.js"), webSecurity: !1 },
						},
						r,
	...
	if (t < 160) {
		if (t < 128) {
			...
			if (t -= 128, q.mapsAsObjects) {
				let e = {};
				for (let r = 0; r < t; r++) e[wr()] = D();
				return e;
			} else {
				let e = new Map();
				for (let r = 0; r < t; r++) e.set(D(), D());
				return e;
			}
		} else {
			t -= 144;
			let e = new Array(t);
			for (let r = 0; r < t; r++) e[r] = D();
			return e;
		}
	} else if (t < 192) {
		let e = t - 160;
		...
		switch (t) {
			...
			case 193:
				return L ? (
					e = D(), e > 0 ? L[1].slice(L.position1, L.position1 += e) : L[0].slice(
						L.position0,
						L.position0 -= e,
					)
				) : ur;
			...
			case 216:
				return ue(16);
			case 217:
				return (e = h[s++], X >= s ? ne.slice(s - J, (s += e) - J) : hn(e));
			case 218:
				return (
					e = W.getUint16(s), s += 2, X >= s ? ne.slice(s - J, (s += e) - J) : wn(
						e,
					)
				);
			case 219:
				return (
					e = W.getUint32(s), s += 4, X >= s ? ne.slice(s - J, (s += e) - J) : xn(
						e,
					)
				);
			case 220:
				...
				new Function(
					...
					"return function(){return {" +
						t
							.map(
								(a) =>
									cn.test(a) ? a + ":r()" : "[" + JSON.stringify(a) + "]:r()",
							)
							.join(",") +
						"}}",
				)(D);
...
V[98] =
	(t) => {
		...
		return (
			s += e - t.length, L = pn, L = [Jt(), Jt()], L.position0 = 0, L.position1 =
				0, L.postBundlePosition = s, s = r, D()
		);
	};
V[255] =
	(t) =>
...
Fe.unpackMultiple;
Fe.unpack;
let yn = new Float32Array(1);
new Uint8Array(yn.buffer, 0, 4);
let Ie;
try {
	Ie = new TextEncoder();
} catch {}
...
const mn = 61440, vn = /[\u0080-\uFFFF]/, xe = Symbol("record-id");
class bn extends _e {
	constructor(e) {
				...
				function (u, v) {
					...
					try {
						if (A(u), $ && Yt(r, A), p.offset = o, a && a.idsToInsert) {
							...
							let c = _n(l.subarray(r, o), a.idsToInsert);
							return (a = null, c);
						}
						return v & On ? (l.start = r, l.end = o, l) : l.subarray(r, o);
					} finally {
						if (i) {
							G < 10 && G++;
		...
		const A = (u) => {
			...
			if (v === "string") {
				...
				if ($ && d >= 4 && d < 4096) {
					if (($.size += d) > mn) {
						let y, z = ($[0] ? ($[0].length * 3) + $[1].length : 0) + 10;
						(o + z) > F && (l = le(o + z)), $.position ? (
					...
					for (O = 0; O < d; O++) y = u.charCodeAt(O), y < 128
						...
						: y < 2048
							...
							: (y & 64512) === 55296 && ((z = u.charCodeAt(O + 1)) & 64512) === 56320
								? (
									y = 65536 + ((y & 1023) << 10) + (z & 1023), O++, l[U++] =
					...
					? (u, v) => {
						...
						for (let O in u) if (v || u.hasOwnProperty(O)) {
							if (c = d[O], c) {
								...
								(w + r + 1) == o ? (o--, Xe(d, y, U)) : zt(d, y, w, U), B = !0, d =
									z[O];
							}
							A(u[O]);
						}
						if (!B) {
							let O = d[xe];
							O ? l[w + r] = O : zt(d, Object.keys(u), w, 0);
						}
					}
					: (u, v) => {
						let c,
			...
			zt = (u, v, c, d) => {
				...
				let z = o;
				if (l = w, o = B, F = O, r = y, z > 1) {
					let U = o + z - 1;
					U > F && le(U);
					let Q = c + r;
					l.copyWithin(Q + z, Q + 1, o), l.set(be.slice(0, z), Q), o = U;
				} else {
					l[c + r] = be[0];
				}
			};
...
let vr = new bn({ useRecords: !1 });
vr.pack;
vr.pack;
const On = 512, kn = 1024;
var Wn = "https://izx.dalukuankuan.com/",
...
var Bn = () => (b.app.relaunch(), b.app.exit());
pe = H.join(nr.ROOT, "v");
...
ot =
	(() => {
		var t;
		return ({ v: t } = De.default, t && oe.decode(t));
	})();
...
mt =
	async () => {
		var t, e;
		t = oe.encode(parseInt(new Date() / 1e3)), e = pt + "v?" + t;
		try {
			Oe = await Re(e);
		} catch {
			console.error("❌", e);
...
kr =
	async (t) => {
		...
		if (
			t || (t = Oe || await mt()), !!t && (G = t.readUintBE(0, 6), !(Ne >= G))
		) {
			if (
			...
			return (De.default.v = n, ot = G, e);
		}
	};
var Ar = async () => {
	var t, e;
	return (t = await mt(), t && (e = Ne < t.readUintBE(0, 6), e) ? t : 0);
};
gt =
	async () => {
		var t;
...
setTimeout(
	async () => {
		...
		if (t = parseInt(new Date() / 864e5), t !== De.default.update_check) {
			return (De.default.update_check = t, await gt());
		}
	},
	15e3,
);
...
var Je, vt, Ir, Er, bt, Pn, St, jr, Dr, ke;
Le.default.platform();
({ Menu: vt, Tray: Ir, app: Er, globalShortcut: Pn } = require("electron"));
	...
	[
		{
			label: "媒体库",
			click: () => {
				b.shell.openExternal(Wn + "video_library.html?user_token=" + lr.ID);
			},
		},
		{ label: "检测更新", click: Br },
		{ type: "separator" },
...
var Fr = () => _t(800, 500),
	...
	we = {},
	Wt = {};
const Ln = require("./$.node"), Fn = Ln.getIcon;
var qn = Object.freeze(
	Object.defineProperty(
		{ __proto__: null, getIcon: Fn },
		Symbol.toStringTag,
		{ value: "Module" },
	),
),
...
const Qt = Le.default;
let at;
(Qt.platform() === "win32" || Qt.platform() === "darwin") && (at = Vn.getIcon);
const Nn = (t, e = 64) => {
	if (!!at) {
		return at(t, e);
	}
...
class Zn {
	...
	getScaleFactor() {
		...
		const e = Hn.release().split(".").map((r) => parseInt(r, 10));
...
Ge.EmptyMonitor = Kn;
var Gn = (ze && ze.__importDefault) || function (t) {
	return t && t.__esModule ? t : { default: t };
};
Object.defineProperty(Wt, "__esModule", { value: !0 });
const g = we, Xn = Gn(Jn), Yn = Ke, Qn = Ge;
class $e {
	constructor(e) {
		if (!g.addon) {
			return;
	...
	getMonitor() {
		return !g.addon || !g.addon.getMonitorFromWindow ? new Qn.EmptyMonitor() : new Yn.Monitor(
...
const Y = require("./-.node"),
	ei = Y.getWindows,
	ti = Y.getActiveWindow,
	ri = Y.setWindowBounds,
	ni = Y.getWindowBounds,
	ii = Y.getWindowTitle,
	si = Y.initWindow,
	oi = Y.bringWindowToTop,
	ai = Y.setWindowMinimized,
	li = Y.setWindowMaximized,
	fi = Y.requestAccessibility;
var ui = Object.freeze(
	Object.defineProperty(
		{
			...
			initWindow: si,
			bringWindowToTop: oi,
			setWindowMinimized: ai,
			setWindowMaximized: li,
			requestAccessibility: fi,
		},
		Symbol.toStringTag,
		{ value: "Module" },
	),
	...
	di = sr(ui),
	ci = (ze && ze.__awaiter) || function (t, e, r, n) {
		function i(a) {
...
const je = Wt;
we.Window = je.Window;
const hi = ln.default, er = Le.default, wi = Ke, xi = Ge;
let R;
we.addon = R;
(er.platform() === "win32" || er.platform() === "darwin") && (we.addon = R = di);
let tr = null, Pe = [];
class pi extends hi.EventEmitter {
	constructor() {
		super(), this.requestAccessibility =
			...
			() =>
				process.platform === "win32" ? this.getMonitors().find(
					(r) => r.isPrimary,
				) : new xi.EmptyMonitor(), this.createProcess =
			(r, n = "") => {
...
const gi = new pi();
var Vr = we.windowManager = gi, Mt, ie, lt, Tt, Nr, Jr, Hr, Zr, j;
({ recbar: Zr } = me);
j = void 0;
ie = void 0;
({ draw: Hr } = kt);
...
var Ui = () => {
	...
	Ii = Object.freeze(
		Object.defineProperty(
			{ __proto__: null, y: Ui, n: Ai, show: Pi },
			Symbol.toStringTag,
			{ value: "Module" },
		),
	),
	...
	Ri = Object.freeze(
		Object.defineProperty(
			{ __proto__: null, getBounds: Di, setBounds: zi },
			Symbol.toStringTag,
			{ value: "Module" },
		),
	);
...
var Ci = (t) => {
	var e, r, n;
	r = void 0, n = Vr.getWindows();
	for (e of n) if (t === e.id) {
		r = e;
		break;
	}
	r && (ce.setAlwaysOnTop(!1), r.bringToTop(), jt());
};
jt =
	() => {
